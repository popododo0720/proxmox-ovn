...
func TestMemoryRejectsNewReferencesToUnavailableResources(t *testing.T) {
	...
	}, "late-subnet"); !errors.Is(err, ErrConflict) {
	...
	}, "late-port"); !errors.Is(err, ErrConflict) {
...
func TestMemoryStaticRoutesRequireOneRouterAttachment(t *testing.T) {
	store := deterministicStore()
	topology := buildInvariantTopology(t, store)
	...
	if _, err := store.Delete(context.Background(), model.KindRouterInterface, routerInterface.ID, routerInterface.Revision, "route-interface-delete"); !errors.Is(err, ErrConflict) {
	...
	if _, _, err := store.Update(context.Background(), &unreachable, unreachable.Revision, "route-unreachable"); !errors.Is(err, ErrConflict) {
	...
	if _, _, err := store.Update(context.Background(), &managedDefault, managedDefault.Revision, "route-default"); !errors.Is(err, ErrConflict) {
