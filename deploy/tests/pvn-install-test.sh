...
printf 'test deb payload\n' > "$ASSETS/pvn-node_0.2.2_amd64.deb"
make_manifest() {
    (
        cd "$ASSETS"
        sha256sum pvn-node_0.2.2_amd64.deb pvn-cluster-install \
...
run_bootstrap() {
    PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.2 \
...
reset_logs
if PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.2 \
...
grep -q '/pvn-node_0.2.2_amd64.deb ' "$CURL_LOG" || fail "versioned DEB was not downloaded"
...
reset_logs
if PVN_RELEASE_BASE_URL=http://releases.example.invalid/v0.2.2 \
...
cp "$ASSETS/pvn-node_0.2.2_amd64.deb" "$WORK/deb.good"
...
assert_temp_cleaned
cp "$WORK/deb.good" "$ASSETS/pvn-node_0.2.2_amd64.deb"
make_manifest
...
reset_logs
printf '\n' | script -qefc \
    "PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.2 bash -c \"\$(cat '$BOOTSTRAP')\"" \
...
printf '%s\n' "$IDENTITY" | script -qefc \
    "PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.2 PVN_INVENTORY='$INVENTORY' PVN_PHASE=preflight bash -c \"\$(cat '$BOOTSTRAP')\"" \
...
reset_logs
printf '\n' | script -qefc \
    "PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.2 PVN_INVENTORY='$INVENTORY' PVN_IDENTITY='$IDENTITY' bash -c \"\$(cat '$BOOTSTRAP')\"" \
