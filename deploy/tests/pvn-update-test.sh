...
make_manifest() {
    (
        cd "$ASSETS"
        sha256sum pvn-node_0.2.12_amd64.deb pvn-cluster-update \
...
cp "$ASSETS/pvn-node_0.2.12_amd64.deb" "$WORK/good.deb"
printf 'tampered\n' > "$ASSETS/pvn-node_0.2.12_amd64.deb"
reset_logs
...
cp "$WORK/good.deb" "$ASSETS/pvn-node_0.2.12_amd64.deb"
make_manifest
