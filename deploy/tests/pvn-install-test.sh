...
printf 'test deb payload\n' > "$ASSETS/pvn-node_0.2.5_amd64.deb"
make_manifest() {
    (
        cd "$ASSETS"
        sha256sum pvn-node_0.2.5_amd64.deb pvn-cluster-install \
...
run_bootstrap() {
    PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 \
...
reset_logs
if PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 \
...
grep -q '/pvn-node_0.2.5_amd64.deb ' "$CURL_LOG" || fail "versioned DEB was not downloaded"
...
reset_logs
if PVN_RELEASE_BASE_URL=http://releases.example.invalid/v0.2.5 \
...
cp "$ASSETS/pvn-node_0.2.5_amd64.deb" "$WORK/deb.good"
...
assert_temp_cleaned
cp "$WORK/deb.good" "$ASSETS/pvn-node_0.2.5_amd64.deb"
make_manifest
...
reset_logs
printf '\n' | script -qefc \
    "pvn_bootstrap=\$($BIN/public-curl -fsSL https://releases.example.invalid/pvn-install.sh) && PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 bash -c \"\$pvn_bootstrap\"" \
...
printf '%s\n' "$IDENTITY" | script -qefc \
    "pvn_bootstrap=\$($BIN/public-curl -fsSL https://releases.example.invalid/pvn-install.sh) && PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 PVN_INVENTORY='$INVENTORY' PVN_PHASE=preflight bash -c \"\$pvn_bootstrap\"" \
...
reset_logs
printf '\n' | script -qefc \
    "pvn_bootstrap=\$($BIN/public-curl -fsSL https://releases.example.invalid/pvn-install.sh) && PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 PVN_INVENTORY='$INVENTORY' PVN_IDENTITY='$IDENTITY' bash -c \"\$pvn_bootstrap\"" \
...
if printf 'lab-cluster\ny\n' | script -qefc \
    "pvn_bootstrap=\$($BIN/public-curl -fsSL https://releases.example.invalid/pvn-install.sh) && PVN_RELEASE_BASE_URL=https://releases.example.invalid/v0.2.5 PVN_CP_MEMBERS='$MEMBERS' PVN_TOPOLOGY_BIN='$BIN/pvn-topology' PVN_CONTROL_PLANE_BIN='$BIN/pvn-control-plane' bash -c \"\$pvn_bootstrap\"" \
